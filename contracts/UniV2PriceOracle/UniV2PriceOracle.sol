...
contract UniV2PriceOracle is IPriceOracle {
     ...
     */
    constructor(address _inToken, uint256 _minimumUpdateInterval) {
        ...
        returns (uint256 _amountOut)
    {
        ...
        if (TOKEN == pair.token0()) {
            _amountOut = _divide(reserves1, reserves0, 18); // keep 18-decimal precision
        } else {
            require(TOKEN == pair.token1(), "INVALID PAIR");
            _amountOut = _divide(reserves0, reserves1, 18);
        }
    }
